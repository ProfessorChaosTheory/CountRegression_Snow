...
pre_winter_data <- FitchburgDailyData |>
  ...
  # Filter for May through December 21
  filter(
    (month >= 5 & month <= 11) | 
    (month == 12 & day <= 21)
  ) |>
  ...
  summarise(
    ...
    # Total snowfall May-Dec 21 (unusual but might happen)
...
# Step 3: Count snow days for remainder of winter (December 22 - April 30)
winter_snow_days <- FitchburgDailyData |>
  ...
  # Create Winter season and label
  filter(
    (month == 12 & day >= 22) | 
    (month >= 1 & month <= 4)
  ) |>
  ...
  # Define snow day: snowfall > 4" OR apparent temp mimimum < -30°F.
  mutate(
...
```{r}
#|label: explore snow days per season
# #| echo: false
...
```{r}
snow_days |>
  select(year, snow_days_count, oni_oct_dec, ao_oct_dec, pna_oct_dec) |>
  pivot_longer(cols = c(oni_oct_dec, ao_oct_dec, pna_oct_dec),
...
snow_days |>
  ...
  mutate(snow_days_category = cut(snow_days_count, 
                                   ...
                                   labels = c("0-2 days", "3-4 days", 
                                             "5+ days"))) |>
  pivot_longer(cols = c(total_rainfall_preseason, total_snowfall_preseason,
                        total_precip_preseason, temp_range_preseason),
               names_to = "preseason_metric",
               values_to = "metric_value") |>
  mutate(preseason_metric = case_when(
    preseason_metric == "total_rainfall_preseason" ~ "Total Rainfall (May-Dec 21)",
    preseason_metric == "total_snowfall_preseason" ~ "Total Snowfall (May-Dec 21)",
...
model1 <- poisson_spec |>
  fit(snow_days_count ~ ao_oct_dec * dec_avg_temp, family = poisson,
      data = snow_days)
...
```{r}
#| label: Working Poisson regression model
...
test_stat <- dev1 - dev2
1 - pchisq(test_stat, 2) #degrees of freedom is the number of new terms
```
```{r}
...
test_stat <- dev1 - dev2
1 - pchisq(test_stat, 6) #degrees of freedom is the number of new terms
```
...
evaluate_count_model <- function(model, test_data, model_name) {
  ...
  # Print summary
  cat("\n=== Model:", model_name, "===\n")
  cat("MAE (Mean Absolute Error):", round(mae, 2), "snow days\n")
  cat("RMSE (Root Mean Squared Error):", round(rmse, 2), "snow days\n")
...
results <- evaluate_count_model(model1, snow_days, "Poisson: AO * Dec Temp")

# View the metrics
results$metrics

# View predictions
head(results$predictions, 20)

# Plot actual vs predicted
ggplot(results$predictions, aes(x = actual, y = predicted)) +
  geom_point(alpha = 0.6, size = 3, color = "steelblue") +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed", size = 1) +
  labs(title = "Actual vs Predicted Snow Days (Test Set)",
       subtitle = "Points should fall on red line for perfect predictions",
       x = "Actual Snow Days",
       y = "Predicted Snow Days")

# Plot residuals
ggplot(results$predictions, aes(x = predicted, y = residual)) +
  geom_point(alpha = 0.6, color = "steelblue") +
  geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
  geom_smooth(se = TRUE, color = "blue") +
  labs(title = "Residual Plot",
       subtitle = "Residuals should be randomly scattered around zero",
       x = "Predicted Snow Days",
